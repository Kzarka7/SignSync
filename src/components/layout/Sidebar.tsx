...
import { LayoutDashboard, Video, History, BookOpen, Settings } from 'lucide-react'

const navItems = [
  { to: '/dashboard', label: 'Dashboard', icon: LayoutDashboard },
  { to: '/live', label: 'Live conversation', icon: Video, live: true },
        ...
        <NavLink
          ...
          className={({ isActive }) =>
            `flex items-center gap-3 px-3 py-2.5 rounded-[10px] text-sm font-medium transition-colors ${
              isActive ? 'bg-signal-light text-trust' : 'text-[#C9DDEF] hover:bg-white/5 hover:text-white'
            }`
          }
        >
          <Icon size={18} className="flex-shrink-0" />
          {label}
          {live && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-success shadow-[0_0_0_3px_rgba(31,170,89,0.25)]" />}
        </NavLink>
      ))}

      <div className="mt-auto p-3.5 rounded-xl bg-white/5 text-xs text-[#AFCBE4] leading-relaxed">
        <b className="block text-[13px] text-[#EAF3FB] font-display mb-0.5">Device status</b>
