...
export default function SessionHistoryCard({ session }: { session: ConversationSession }) {
  return (
    <div className="flex items-center gap-4 p-4 border border-border rounded-xl2 bg-white mb-2.5">
      <div className="w-13 h-13 rounded-[10px] bg-signal-light text-signal flex items-center justify-center flex-shrink-0" style={{ width: 52, height: 52 }}>
        <Video size={20} />
      </div>
      <div className="flex-1">
        <div className="text-[13.5px] font-semibold">{session.title}</div>
